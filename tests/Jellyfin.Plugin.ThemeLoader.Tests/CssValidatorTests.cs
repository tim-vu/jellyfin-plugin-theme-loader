...
public class CssValidatorTests
{
    ...
    [Fact]
    public void Validate_DoesNotRewriteBackslashEscapedPaths()
    {
        HashSet<string> files = ["style.css", "assets/bg.png"];

        Assert.Throws<FileNotFoundException>(() => CssValidator.Validate(".x{background:url('assets\\bg.png')}", "style.css", files));
    }
